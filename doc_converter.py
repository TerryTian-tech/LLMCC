...
def convert_doc_to_docx(input_path, output_folder, log_callback=None, is_cancelled_callback=None):
    ...
    if not HAS_WIN32:
        log("错误：未安装 pywin32，无法转换 DOC 文件。请先安装: pip install pywin32")
        return False
...
class DocxTraditionalSimplifiedConverter:
    def __init__(self, converter, direction, log_callback, is_cancelled_callback,
                 preserve_format=True, convert_footnotes=True):
        """
        初始化转换器
        :param converter: ts_converter.Converter 实例
        :param direction: 转换方向 's2t' 或 't2s'
        """
    ...
    def convert_text(self, text):
        ...
        if text and isinstance(text, str):
            return self.converter.convert(text, self.direction)
        return text
    ...
    def _convert_xml_file(self, xml_path):
        ...
        try:
            ...
            def convert_wt_text(match):
                ...
                if text and any('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in text):
    ...
    def _convert_paragraph_simple(self, paragraph):
        ...
        # 如果整个段落没有中文，跳过转换
        if not any('\u4e00' <= c <= '\u9fff' or '\u3400' <= c <= '\u4dbf' for c in full_text):
            return
...
def convert_docx_file(input_path, output_folder, direction, converter,
                      preserve_format=True, convert_footnotes=True,
