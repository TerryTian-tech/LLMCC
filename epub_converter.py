...
def _convert_xhtml_bytes(raw: bytes, converter: Converter, direction: str,
                         log_callback: Optional[Callable]) -> bytes:
    """
    转换单个 XHTML 文档的原始字节。
    编码检测 → BeautifulSoup 解析 → 文本节点批量转换 → 序列化回字节。
    """
    ...
    _convert_dom_text(soup, converter, direction)
    ...
    try:
        ...
        for info in zin.infolist():
            ...
            try:
                ...
                converted[file_name] = _convert_xhtml_bytes(raw, converter, direction, log_callback)
