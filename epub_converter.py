...
def _convert_xhtml_item(item, converter: Converter, direction: str, log_callback: Optional[Callable]) -> None:
    """
    转换单个 XHTML 内容项。
...
def convert_epub_file(
    ...
    for item in book.get_items():
        ...
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
            ...
            try:
                _convert_xhtml_item(item, converter, direction, log_callback)
            except Exception as e:
